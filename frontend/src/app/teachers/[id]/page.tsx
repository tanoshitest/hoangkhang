...
export default function TeacherDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!teacher) return <div className="text-center py-12 text-gray-500">Không tìm thấy giáo viên</div>;
  ...
  return (
    <div className="py-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        <Link href="/teachers" className="inline-flex items-center text-sm text-gray-500 hover:text-gray-700">
        ...
        <div className="mt-4 md:flex md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">
              {teacher.name} <span className="text-base font-normal text-gray-500">({teacher.code})</span>
            </h1>
          ...
          <Link href="/payroll" className="mt-4 md:mt-0 inline-flex items-center px-4 py-2 rounded-md text-sm font-medium text-white bg-blue-600 hover:bg-blue-700">
        ...
        <div className="mt-6">
          {activeTab === 'info' && (
            <div className="bg-white shadow rounded-lg p-6">
              <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                <div><dt className="text-sm text-gray-500">Điện thoại</dt><dd className="text-sm font-medium text-gray-900">{teacher.phone}</dd></div>
                <div><dt className="text-sm text-gray-500">Email</dt><dd className="text-sm font-medium text-gray-900">{teacher.email || '-'}</dd></div>
                <div><dt className="text-sm text-gray-500">Trình độ</dt><dd className="text-sm font-medium text-gray-900">{teacher.education || '-'}</dd></div>
                <div><dt className="text-sm text-gray-500">Chứng chỉ</dt><dd className="text-sm font-medium text-gray-900">{teacher.certificates || '-'}</dd></div>
              </dl>
            </div>
          )}
          ...
          {activeTab === 'availability' && (
            <div className="bg-white shadow rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">Khung giờ có thể dạy</h3>
              ...
              {showAvailForm && (
                <form onSubmit={addAvailability} className="mb-4 bg-blue-50 rounded-md p-4 grid grid-cols-4 gap-3 items-end">
                  <div>
                    <label className="block text-xs text-gray-600">Thứ</label>
                  ...
                  <div>
                    <label className="block text-xs text-gray-600">Từ</label>
                    <input type="time" value={availForm.startTime} onChange={(e) => setAvailForm({ ...availForm, startTime: e.target.value })}
                      className="mt-1 block w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-600">Đến</label>
                    <input type="time" value={availForm.endTime} onChange={(e) => setAvailForm({ ...availForm, endTime: e.target.value })}
                      className="mt-1 block w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm" />
                  </div>
                  <button type="submit" className="px-3 py-1.5 bg-blue-600 text-white rounded-md text-sm">Lưu</button>
                </form>
              )}

              {teacher.availability.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-6">Chưa có khung giờ nào</p>
              ) : (
                <ul className="divide-y divide-gray-200">
                  {teacher.availability.map((a) => (
                    <li key={a.id} className="py-3 flex items-center justify-between">
                      ...
                      <button onClick={() => deleteAvailability(a.id)} className="text-gray-400 hover:text-red-500">
          ...
          {activeTab === 'rates' && (
            <div className="bg-white shadow rounded-lg p-6">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-medium text-gray-900">Đơn giá (lịch sử không bị ghi đè)</h3>
              ...
              {showRateForm && (
                <form onSubmit={addRate} className="mb-4 bg-blue-50 rounded-md p-4 grid grid-cols-2 gap-3 sm:grid-cols-5 items-end">
                  <div>
                    <label className="block text-xs text-gray-600">Loại lớp</label>
                  ...
                  <div>
                    <label className="block text-xs text-gray-600">Vai trò</label>
                  ...
                  <div>
                    <label className="block text-xs text-gray-600">Đơn giá/giờ *</label>
                    <input type="number" required min="1" value={rateForm.rate}
                      onChange={(e) => setRateForm({ ...rateForm, rate: e.target.value })}
                      className="mt-1 block w-full border border-gray-300 rounded-md px-2 py-1.5 text-sm" />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-600">Hiệu lực từ *</label>
              ...
              {teacher.rates.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-6">Chưa có đơn giá nào</p>
              ) : (
                <table className="min-w-full divide-y divide-gray-200">
                  <thead>
                    <tr>
                      <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Loại lớp</th>
                      <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Vai trò</th>
                      <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Đơn giá</th>
                      <th className="px-3 py-2 text-left text-xs font-medium text-gray-500">Hiệu lực</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200">
                    {teacher.rates.map((r) => (
                      <tr key={r.id}>
                        <td className="px-3 py-2 text-sm text-gray-900">{r.classType || 'Tất cả'}</td>
                        <td className="px-3 py-2 text-sm text-gray-900">{ROLE_LABELS[r.role] || r.role}</td>
                        <td className="px-3 py-2 text-sm font-medium text-gray-900">{formatVND(r.rate)}/h</td>
          ...
          {activeTab === 'classes' && (
            <div className="bg-white shadow rounded-lg p-6">
              {teacher.classesMain.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-6">Chưa phụ trách lớp nào</p>
              ) : (
                <ul className="divide-y divide-gray-200">
                  {teacher.classesMain.map((c) => (
                    <li key={c.id} className="py-3">
                      <Link href={`/classes/${c.id}`} className="text-sm font-medium text-blue-600 hover:underline">
                        {c.code}
                      </Link>
                      <span className="text-sm text-gray-500 ml-2">{c.course.name} • {c.status}</span>
                    </li>
                  ))}
                </ul>
              )}
          ...
          {activeTab === 'sessions' && (
            <div className="bg-white shadow rounded-lg p-6">
              {teacher.sessions.length === 0 ? (
                <p className="text-sm text-gray-500 text-center py-6">Chưa có buổi dạy nào</p>
              ) : (
                <ul className="divide-y divide-gray-200">
                  {teacher.sessions.map((s) => (
                    <li key={s.id} className="py-3 flex items-center justify-between">
                      <div>
                        <Link href={`/sessions/${s.id}`} className="text-sm font-medium text-blue-600 hover:underline">
                          {s.class.code}
                        </Link>
                      ...
                      <span className="text-xs text-gray-500">{SESSION_STATUS[s.status] || s.status}</span>
                    </li>
                  ))}
                </ul>
              )}
