...
const TYPE_LABELS: Record<string, { label: string; color: string }> = {
  lead_reminder: { label: 'Lead', color: 'bg-blue-100 text-blue-800' },
        ...
        <div className="mt-6 bg-white shadow overflow-hidden sm:rounded-md">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-sm text-gray-500">Chưa có thông báo nào</p>
              <p className="text-xs text-gray-400 mt-1">Nhấn "Quét thông báo" để kiểm tra các quy tắc cảnh báo</p>
            </div>
          ) : (
            <ul className="divide-y divide-gray-200">
              {notifications.map((n) => (
                <li key={n.id}
                  className={`px-4 py-4 sm:px-6 ${!n.isRead ? 'bg-blue-50' : ''} ${!n.isRead ? 'cursor-pointer' : ''}`}
                  onClick={() => !n.isRead && markRead(n.id)}>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_LABELS[n.type]?.color || 'bg-gray-100'}`}>
                          {TYPE_LABELS[n.type]?.label || n.type}
                        </span>
                        {!n.isRead && <span className="w-2 h-2 rounded-full bg-blue-500" />}
                      </div>
                      <p className={`text-sm mt-1 ${!n.isRead ? 'font-medium text-gray-900' : 'text-gray-700'}`}>{n.title}</p>
                      <p className="text-sm text-gray-500 mt-0.5">{n.content}</p>
                    </div>
                    <span className="text-xs text-gray-400 ml-4 whitespace-nowrap">{formatDate(n.createdAt)}</span>
                  </div>
                </li>
              ))}
            </ul>
