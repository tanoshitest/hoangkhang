  ...
  if (!course) {
    return (
      <div className="text-center py-12">
        <h3 className="text-sm font-medium text-gray-900">Khóa học không tồn tại</h3>
        <button
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
              ...
              <dl className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                {course.textbook && (
                  <div>
                    ...
                    <dd className="mt-1 text-sm text-gray-900">{course.textbook}</dd>
                  </div>
                )}
                {course.goal && (
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Mục tiêu</dt>
                    <dd className="mt-1 text-sm text-gray-900">{course.goal}</dd>
                  </div>
                )}
                {course.completionStandard && (
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Chuẩn đầu ra</dt>
                    <dd className="mt-1 text-sm text-gray-900">{course.completionStandard}</dd>
                  </div>
                )}
                {course.testSchedule && (
                  <div>
                    <dt className="text-sm font-medium text-gray-500">Lịch kiểm tra</dt>
                    <dd className="mt-1 text-sm text-gray-900">{course.testSchedule}</dd>
                  </div>
                )}
                {course.content && (
                  <div className="md:col-span-2">
                    <dt className="text-sm font-medium text-gray-500">Nội dung</dt>
                    <dd className="mt-1 text-sm text-gray-900 whitespace-pre-line">{course.content}</dd>
                  </div>
                )}
              </dl>
            </div>
          ...
          <div className="bg-white shadow sm:rounded-lg">
            ...
            <div className="px-4 py-5 sm:p-6 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium text-gray-900">{course.totalSessions || '—'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500 flex items-center">
                ...
                <span className="text-sm font-medium text-gray-900">{course.totalHours ? `${course.totalHours}h` : '—'}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500 flex items-center">
                ...
                <span className="text-sm font-medium text-blue-600">{formatFee(course.standardFee)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500 flex items-center">
                ...
                <span className="text-sm font-medium text-gray-900">{course.classes.length}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white shadow sm:rounded-md">
          <div className="px-4 py-5 sm:px-6 border-b border-gray-200 flex items-center justify-between">
            <h3 className="text-lg font-medium text-gray-900">Các lớp ({course.classes.length})</h3>
            <button
              ...
              <li key={cls.id}>
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      <div className="text-sm font-medium text-gray-900">{cls.code}</div>
