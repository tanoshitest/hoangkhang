  ...
  return (
    <div className="py-6">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <h1 className="mt-4 text-2xl font-semibold text-gray-900">Công nợ</h1>
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="mt-6">
              <h2 className="text-lg font-medium text-gray-900 mb-3">Chưa đến hạn ({upcoming.length})</h2>
              {upcoming.length === 0 && overdue.length === 0 ? (
                <p className="text-center py-8 text-sm text-gray-500 bg-white rounded-lg shadow">Không có công nợ nào</p>
              ) : (
  ...
  return (
    <div className="bg-white shadow overflow-hidden sm:rounded-md">
      <ul className="divide-y divide-gray-200">
        {debts.map((d) => (
          <li key={d.id} className="px-4 py-4 sm:px-6 hover:bg-gray-50 cursor-pointer" onClick={() => onClick(d.id)}>
            <div className="flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-gray-900">{d.student.name}</span>
                  <span className="text-xs text-gray-500">({d.student.code})</span>
                  {d.isOverdue && (
