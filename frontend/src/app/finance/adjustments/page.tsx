        ...
        <div className="mt-6 bg-white border border-slate-200 shadow-sm overflow-hidden sm:rounded-xl">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-sm text-slate-500">Không có điều chỉnh nào</p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-slate-900">{TYPE_LABELS[a.type]}</span>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS[a.status]?.color}`}>
                          {STATUS[a.status]?.label}
                        </span>
                        <span className="text-sm font-semibold text-slate-900">{formatVND(a.amount)}</span>
                      </div>
                      <p className="text-sm text-slate-500 mt-1">
                        {a.receivable.student.name} ({a.receivable.student.code}) • {a.receivable.course.name}
                      </p>
                      <p className="text-xs text-slate-400 mt-1">{a.reason} • {formatDate(a.createdAt)}</p>
                    </div>
                    {a.status === 'pending' && (
